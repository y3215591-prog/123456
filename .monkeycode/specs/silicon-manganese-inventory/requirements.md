...
3. 出库表单 SHALL 包含：出库日期（默认当天）、销售订单号（下拉选择，支持输入关键词搜索）、客户（自动带出，不可编辑）、品名规格（自动带出）、数量(吨)、操作人、备注。
4. WHEN 用户选择或输入销售订单号，系统 SHALL 自动查询并填充：客户名称、品名规格、订单数量、交货截止日期、提货方式等信息。
5. 出库表单 SHALL 在客户字段旁提供「新增客户」快捷按钮，点击后弹出客户新增表单（字段同需求12.4），新增成功后立即刷新客户下拉列表。
...
3. WHEN 手动新增时用户输入销售订单号，系统 SHALL 自动带出客户名称、物料名称、规格等信息。
4. 用户 SHALL 可以编辑客户收货净重和备注字段。
